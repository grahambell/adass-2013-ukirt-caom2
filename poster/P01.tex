...
\definecolor{u2cgreen}{RGB}{0,125,0}
\definecolor{u2cyellow}{RGB}{255,255,125}
\definecolor{listingfaint}{gray}{0.70}
\definecolor{metadatagreen}{RGB}{75,255,75}
...
\headerbox{Introduction}{name=intro,column=0,row=0}{
    ...
    \begin{itemize}
        ...
        \item The raw data had already
        been read from archive tapes
        and organized on disk.
...
\headerbox{Data Reduction}{name=pipeline,column=2,span=1,below=intro,above=bottom}{
    ...
    \begin{itemize}
    \item ORAC-DR has already been used for these instruments (except CGS3).
    \end{itemize}
    ...
    \item Currently testing ORAC-DR on a range of data.
    For observations pre-dating its introduction (around 1999):
...
\headerbox{Example Pipeline Image}{name=example,column=0,span=2,below=intro,above=bottom}{
    \raggedright

    \begin{itemize}[topsep=0pt]
    \item
    Separate raw files merged into one file per observation for e-transfer / pipeline processing.

    \item
    Observation grouping and recipe headers determined from the intermediate database.

    \item
    Processed with the normal \texttt{JITTER\_SELF\_FLAT} recipe in ORAC-DR.
    \end{itemize}


    \begin{tabular}{c}
    \includegraphics[scale=0.7,angle=270]{../gfx/hh111} \\
    \textbf{HH\,111 protostellar outflows observed with IRCAM3 in 1997.}
    \end{tabular}

    \vspace{0.05in}

    Observed as part of a multi-year campaign studying the
    proper motions (measuring 265--461 km/s)
    and variability of the knots.
    Found a number of faint features
...
\quad \quad \= <\textcolor{listingfaint}{caom2:}collection>\textbf{UKIRT}</\textcolor{listingfaint}{caom2:}collection> \\
            ...
            \> <\textcolor{listingfaint}{caom2:}metaRelease>\tikz[baseline]{\node[anchor=base,fill=yellow, text width=] (xmldate) {\smaller \bf 1999-02-01T23:59:59.000};}</\textcolor{listingfaint}{caom2:}metaRelease> \\
...
\draw[->,>=stealth,metadatagreen] (mongofile.east) to[out=0,in=180] (xmlid.west);
\draw[->,>=stealth,metadatagreen] (mongoobject.east) to[out=0,in=180] (xmlobject.west);
\draw[->,>=stealth,metadatagreen] (mongotype.east) to[out=0,in=180] (xmltype.west);
\draw[->,>=stealth,metadatagreen] (mongomode.east) to[out=0,in=100] (xmlmode.north);
\draw[->,>=stealth,metadatagreen] (mongoobs.east) to[out=0,in=270] (xmlobs.south);
\draw[->,>=stealth,metadatagreen] (mongodate.east) to[out=0,in=270] (xmldate.south);

\node (mongobrace) at ($(mongoamstart.east)!0.5!(mongoamend.east) + (1ex,0)$) {\textcolor{metadatagreen}{\}}};
\draw[->,>=stealth,metadatagreen] (mongobrace.east) to[out=350,in=120] (xmlelevation.north);
